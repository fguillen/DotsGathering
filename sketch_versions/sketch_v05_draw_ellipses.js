...
function setup() {
  ...
  ellipseNoise = new NoiseGenerator(3, 10);
...
class Utils {
  ...
  static drawNoisedEllipse(position, points, hight, width, angle, noiseSeed, color, blendModeCode, noiseGenerator){
...
class Dot {
    ...
    for (let index = 0; index < 6; index++) {
      ...
      if(
        ...
        if(rack[rackPosition.y][rackPosition.x] == null) {
          ...
          const noisePosition = Utils.vectorRandomNoise(3);
  ...
  draw() {
    ...
    const noiseSeed = this.rackPosition.x + this.rackPosition.y;
    Utils.drawNoisedEllipse(this.position, 10, DOTS_RADIUS / 2, DOTS_RADIUS / 2, 0, noiseSeed, _color, MULTIPLY, ellipseNoise);
    fill("purple")
